...
public class MenuButton {
    ...
    private double areaLocalX;
    private double areaLocalY;
    ...
    public double getAreaLocalX() { return areaLocalX; }
    public double getAreaLocalY() { return areaLocalY; }
    public void setAreaLocalX(double x) { this.areaLocalX = x; }
    public void setAreaLocalY(double y) { this.areaLocalY = y; }

    public boolean isVisible() { return visible; }

    public void setVisible(boolean visible, Player player) {
        if (this.visible == visible) return;
        this.visible = visible;
        int eid = getEntityId();
        if (visible) {
            float size = (float) config.getDouble("size", 1.0);
            FakeEntityFactory.setDisplayScale(player, eid, size, size, size);
        } else {
            FakeEntityFactory.setDisplayScale(player, eid, 0f, 0f, 0f);
        }
    }
}
